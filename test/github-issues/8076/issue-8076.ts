    ...
    it("should return descendants tree without member relations", async () =>
        await Promise.all(
            connections.map(async (connection) => {
                ...
                expect(
                    result.childCategories[0].childCategories[0].title,
                ).equals("Category 1.1.1")
                expect(result.childCategories[0].childCategories[0].members).to
                    .be.undefined
            }),
        ))
    ...
    it("should return descendants tree with member relations", async () =>
        await Promise.all(
            connections.map(async (connection) => {
                ...
                expect(
                    result.childCategories[0].childCategories[0].members,
                ).to.be.an("array")
                expect(
                    result.childCategories[0].childCategories[0].members,
                ).to.have.lengthOf(0)
    ...
    it("should return ancestors tree without member relations", async () =>
        await Promise.all(
            connections.map(async (connection) => {
                ...
                expect(result.parentCategory!.parentCategory!.title).equals(
                    "Category 1",
                )
                expect(result.parentCategory!.parentCategory!.members).to.be
                    .undefined
            }),
        ))
    ...
    it("should return ancestors tree with member relations", async () =>
        await Promise.all(
            connections.map(async (connection) => {
                ...
                expect(result.parentCategory!.parentCategory!.members).to.be.an(
                    "array",
                )
                expect(
                    result.parentCategory!.parentCategory!.members,
                ).to.have.lengthOf(1)
                expect(
                    result.parentCategory!.parentCategory!.members[0].title,
                ).equals("Test")
            }),
        ))
})
