    ...
    it("should load relation count on owner side with additional conditions", () =>
        Promise.all(
            connections.map(async (connection) => {
                ...
                expect(
                    loadedPosts![0].categories[0].removedImageCount,
                ).to.be.equal(1)
                ...
                expect(loadedPost!.categories[0].removedImageCount).to.be.equal(
                    1,
                )
            }),
        ))
