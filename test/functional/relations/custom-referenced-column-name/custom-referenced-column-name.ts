        ...
        it("should load related entity when relation use custom referenced column name", () =>
            Promise.all(
                connections.map(async (connection) => {
                    ...
                    expect(loadedPosts![1].categoryName).to.be.equal(
                        "airplanes",
                    )
        ...
        it("should load related entity when relation defined with empty join column", () =>
            Promise.all(
                connections.map(async (connection) => {
                    ...
                    expect(
                        loadedPosts![0].categoryWithEmptyJoinCol.id,
                    ).to.be.equal(1)
                    expect(
        ...
        it("should load related entity when relation defined without column name", () =>
            Promise.all(
                connections.map(async (connection) => {
                    ...
                    expect(
                        loadedPosts![0].categoryWithoutColName.id,
                    ).to.be.equal(1)
                    expect(
            ...
            Promise.all(
                connections.map(async (connection) => {
                    ...
                    expect(loadedPosts![0].categoryWithoutRefColName2).to.not.be
                        .undefined
                    expect(
                        loadedPosts![0].categoryWithoutRefColName2.id,
                    ).to.be.equal(1)
                    expect(loadedPosts![1].categoryWithoutRefColName2).to.not.be
                        .undefined
                    expect(
        ...
        it("should load related entity when relation defined without column name", () =>
            Promise.all(
                connections.map(async (connection) => {
                    ...
                    expect(loadedPosts![1].tagWithEmptyJoinCol.id).to.be.equal(
                        2,
                    )
            ...
            Promise.all(
                connections.map(async (connection) => {
                    ...
                    expect(loadedPosts![0].tagWithoutRefColName2).to.not.be
                        .undefined
                    expect(
                        loadedPosts![0].tagWithoutRefColName2.id,
                    ).to.be.equal(1)
                    expect(loadedPosts![1].tagWithoutRefColName2).to.not.be
                        .undefined
                    expect(
